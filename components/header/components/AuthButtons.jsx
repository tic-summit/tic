...
function AuthButtons(mobile) {
    ...
    return (
        <>
            ...
            <Link href={`${user?.userType === "instructor" ? '/instructor/dashboard' : '/student/dashboard'}`} className='px-4 py-2 flex-1 md:flex-0 text-nowrap sm:w-fit rounded-full   border text-brand text-center'>Dashoboard</Link>
             <button onClick={logout} className={`px-4 py-2 flex-1 md:flex-0 text-nowrap sm:w-fit rounded-full  text-center  ${(pathname === '/' && !mobile) ? 'border-white text-white' : mobile && 'bg-brand text-white'}  `}>Log out</ button>
</div>            }
        </>
    )
}
